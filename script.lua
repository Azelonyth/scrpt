--[[
    SAMBUNG KATA AUTO - V17.1 ULTIMATE FIX
    - Perbaikan total deteksi prefix, typing, dan stabilitas.
    - Fitur: Auto Answer, ESP, Trap Engine, Humanizer, AFK Farm, Database Manager.
    - Kompatibel dengan Delta Executor dan executor lainnya.
    - Database kosong, silakan paste sendiri.
]]
...
local Config = {
    ...
    SavedCFrame = nil,
    TargetUIName = "",
    Blacklist = {"ZAT", "SKOR", "SCORE", "MENU", "WAKTU", "TIME", "LEVEL", "RANK", "RECORD", "TEAM", "REWARDS", "PLAYERS", "ROUND", "SETTINGS", "BACK", "CLOSE", "EXIT"}
}
...
-- Membersihkan teks dari rich text dan spasi
local function cleanText(str)
    if not str then return "" end
    local cleaned = str:gsub("<[^>]+>", "") -- hapus rich text
    cleaned = cleaned:gsub("%s+", " ") -- normalisasi spasi
    return cleaned
end

-- Mendapatkan prefix dari teks (misal "Kata: BUKU" -> "BUKU")
local function extractPrefix(teks)
    teks = cleanText(teks):upper()
    -- Cari pola "Kata : ABC" atau "Huruf: D" atau mungkin hanya kata pendek
    local prefix = teks:match(":[%s]*([%a]+)") -- setelah titik dua
    if prefix then return prefix end
    -- Coba ambil kata terakhir jika panjang <= 3 (asumsi itu huruf)
    local words = {}
    ...
    if #words > 0 then
        ...
        if #last <= 3 then return last end
    end
    return nil
end

-- Mendapatkan 5 opsi kata berdasarkan awalan
...
local function ketikOpsi(data)
    ...
    task.wait(1) -- jeda setelah kirim
    State.IsBusy = false
end
...
local Window = Rayfield:CreateWindow({
    Name = "Sambung Kata | ULTIMATE V17.1",
    LoadingTitle = "Optimized by Delta",
...
SetupTab:CreateButton({
    ...
    Callback = function()
            ...
            if gui:IsA("ScreenGui") and gui.Enabled then
                for _, obj in ipairs(gui:GetDescendants()) do
                    if (obj:IsA("TextLabel") or obj:IsA("TextButton")) and obj.Visible and obj.Text ~= "" then
                        ...
                        if not isBlacklisted and (t:find(":") or #t <= 10) then
                            Config.TargetUIName = obj.Name
                            State.LastUITarget = obj
                            Rayfield:Notify({Title="Target ditemukan!", Content=obj.Name})
                            return
                        end
                    end
                end
        ...
        Rayfield:Notify({Title="Tidak ditemukan", Content="Coba manual"})
    end
})
...
SetupTab:CreateInput({
    ...
    Callback = function(v)
        ...
        State.LastUITarget = nil
    end
})

SetupTab:CreateParagraph({
    Title = "Blacklist",
    Content = "Kata yang diabaikan: " .. table.concat(Config.Blacklist, ", ")
})
...
Rayfield:Notify({
    Title = "Script siap!",
    Content = "Silakan paste database dan atur target."
})
